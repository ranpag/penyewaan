    ...
    try {
        ...
        return res.status(200).json({
            ...
            pagination: {
                ...
                totalPage: totalAllDetails > 10 ? Math.floor(totalAllDetails / rentalDetails.length) + 1 : Math.floor(totalAllDetails / rentalDetails.length)
            }
        });
    }
    catch (err) {
    ...
    try {
        ...
        const detail = await prisma.$transaction(async (tx) => {
            ...
            const updatedRentalDetail = await tx.penyewaan_detail.update({
                ...
                data: {
                    ...resultNumberBody,
                    penyewaan_detail_subharga: stockReductionTool.alat_hargaperhari * (resultNumberBody.penyewaan_detail_jumlah || rentalDetails.penyewaan_detail_jumlah)
                }
            });
