...
const existingTools = async (toolIds) => {
    ...
    if (missingToolIds.length > 0) {
        throw new Joi.ValidationError(`ID alat berikut tidak ditemukan: ${missingToolIds.join(", ")}`, [
            {
...
const createRental = {
    body: Joi.object()
        ...
        daftar_alat: Joi.array()
            ...
            .external(async (items) => {
            if (items.length === 0)
                return items;
            const alatIds = items.map((item) => item.alat_id);
